...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(
        ...
        "where (lower(p.label) like lower(concat('%', :query,'%')) or lower(p.brand) like lower(concat('%', :query,'%')))" +
        "and (c.name in :categories or p.color in :colors  or  p.brand in :brands) "
    )
    Page<Product> filteredSearch(
